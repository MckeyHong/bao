...
trait Repository
{
     ...
     *
     * @param  string  $whereField [搜尋條件欄位名稱]
     * @param  string  $whereValue [搜尋條件資訊]
     * @param  array   $field      [要搜尋的欄位, Ex.['id', 'name']]
     * @return boolean
     */
    public function findByWhere($whereField = 'id', $whereValue = '', $field = ['*'])
    {
        return $this->entity::select($field)->where($whereField, $whereValue)->first();
    }
    ...
    */
     ...
     * 查詢帶有多個條件式
     *
     * @param  array  $params  [$key(要搜尋欄位) => $value(要搜尋的資訊)]
     * @param  array  $field
     * @param  bool   $toArray
     * @return mixed
     */
    public function whereMuti($params, $field = ['*'], $toArray = false)
    {
        $entity = $this->entity::select($field);
